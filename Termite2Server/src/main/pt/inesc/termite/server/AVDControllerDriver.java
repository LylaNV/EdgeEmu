...
public class AVDControllerDriver {
    ...
    public String startApp(int emuPort, String appPackage) throws IOException {

        //LYLA
        String exeCmdListOfApps;
        Process process;
        if(platform.equals("windows")) {
            exeCmdListOfApps = sdkPath + File.separator + "platform-tools" + File.separator + "adb -s emulator-" + emuPort + " shell pm list packages";
        } else {
            exeCmdListOfApps = sdkPath + File.separator + "platform-tools" + File.separator + "./adb -s emulator-" + emuPort + " shell pm list packages";
        }
        process = Runtime.getRuntime().exec(exeCmdListOfApps);
        ...
        if (platform.equals("windows")) {
            ...
            if (installed){
                System.out.println("[AVDControllerDriver -> startApp -> $ Application is already installed. Starting it ...");
                //exeCmd = sdkPath + File.separator + "platform-tools" + File.separator + "adb -s emulator-" + emuPort + " shell monkey -p " + appPackage + " -c android.intent.category.LAUNCHER 1";
                exeCmd = sdkPath + File.separator + "platform-tools" + File.separator + "adb -s emulator-" + emuPort + " shell monkey --pct-syskeys 0 -p " + appPackage + " -c android.intent.category.LAUNCHER 1";
                exeCmd = exeCmd.replace(".apk","");
                System.out.println("[AVDControllerDriver -> startApp -> $ starting app command is: " + exeCmd);
                try {
                    // LYLA
                    System.out.println("[AVDControllerDriver -> startApp -> before starting running the app... ");
                    //
                    Runtime.getRuntime().exec(exeCmd);
                    // LYLA
                    System.out.println("[AVDControllerDriver -> startApp -> starting app command is done... ");
                    //
                ...
                System.out.println("[AVDControllerDriver -> startApp -> $ Application is not installed yet!!!");
                String installingResult = null;
                installingResult = installApk(emuPort,appPackage);
                if (installingResult.contains("Success")){
                    System.out.println("[AVDControllerDriver -> startApp -> $ Application is installed, now starting it ...");
                    //exeCmd = sdkPath + File.separator + "platform-tools" + File.separator + "adb -s emulator-" + emuPort + " shell monkey -p " + appPackage + " -c android.intent.category.LAUNCHER 1";
                    exeCmd = sdkPath + File.separator + "platform-tools" + File.separator + "adb -s emulator-" + emuPort + " shell monkey --pct-syskeys 0 -p " + appPackage + " -c android.intent.category.LAUNCHER 1";
                    exeCmd = exeCmd.replace(".apk","");
                    System.out.println("[AVDControllerDriver -> startApp -> $ starting app command is: " + exeCmd);
                    ...
                    System.out.println("[AVDControllerDriver -> startApp -> $ Application installation was not successful ...");
                    System.out.println("[AVDControllerDriver -> startApp -> $ Application installation result: " + installingResult);
                }
            }
            // LYLA
            //System.out.println("[AVDControllerDriver -> startApp -> command to be executed is: " + exeCmd);
            //
        } else {
            //exeCmd = sdkPath + File.separator + "platform-tools" + File.separator + "./adb -s emulator-" + emuPort + " shell monkey -p " + appPackage + " -c android.intent.category.LAUNCHER 1";
            if (installed){
...
//        }
    ...
    public String installApk(int emuPort, String apkName) {
        ...
        // LYLA
        System.out.println("[AVDControllerDriver -> installApk -> $ installing the application named " + apkName + " ...");
        //
        ...
        if (platform.equals("windows")) {
            ...
            // LYLA
            System.out.println("[AVDControllerDriver -> installApk -> command to be executed is: " + exeCmd);
            //
        } else {
            exeCmd = CmdPath + "./adb -s emulator-" + emuPort + " install -t -r " + apkPath;
            // LYLA
        ...
        try {
            // LYLA
            System.out.println("[AVDControllerDriver -> installApk -> before starting installing the app... ");
            //
            System.out.println("Executing install apk cmd: " + exeCmd);
            Process process = Runtime.getRuntime().exec(exeCmd);
            // LYLA
            System.out.println("[AVDControllerDriver -> startApp -> installing app command is done... ");
            //
            BufferedReader stdInput = new BufferedReader(new InputStreamReader(process.getInputStream()));
    ...
    private void setAllRedirections() {
        ...
        if (emulatorsInstances.size() == 0) {
            ...
            for (Emulator emu : emulatorsInstances) {
                // LYLA: 16/01/2023
                System.out.println("[setAllRedirections -> emulator name is: " +emu.get_name());
                System.out.println("[setAllRedirections -> emulator port is: " +emu.get_port());
                //
    ...
    private void setRedirection(int emuPort) {
        ...
        try {
            ...
            // LYLA: 18/01/2023
            System.out.println("[setRedirection -> Testing: before creating reader");
            //

            BufferedReader reader = new BufferedReader(new InputStreamReader(confConnection.getInputStream()));

            // LYLA: 18/01/2023
            //System.out.println("[setRedirection -> reader is: " + reader.readLine());
            //checkAuthentication(reader.readLine(), confConnection, emuPort);
            checkAuthentication(confConnection, emuPort);
            //
    ...
    /*
    * @LYLA
    * In some system there is a need for authentication to access to an emulator console.
    * */
    //private void checkAuthentication(String readerString, Socket confConnection, int emuPort) throws IOException {
    private void checkAuthentication(Socket confConnection, int emuPort) throws IOException {
        BufferedReader reader = new BufferedReader(new InputStreamReader(confConnection.getInputStream()));
        String readerString = reader.readLine();
        if (readerString.contains("auth") || readerString.contains("Authentication")){
            // https://stackoverflow.com/questions/473446/java-current-machine-name-and-logged-in-user
            while (!readerString.contains("emulator_console_auth_token")){
                readerString = reader.readLine();
            }

            String authenticationKeyFilePath = readerString.substring(1, readerString.length()-1);
            String authenticationKey = Files.readString(Path.of(authenticationKeyFilePath));
            System.out.println("Authenticating emulator-" + emuPort);
            String authCommand = "auth" + " " + authenticationKey;
            // write authentication command with authentication key
            BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(confConnection.getOutputStream()));
            writer.write(authCommand);
            writer.flush();

//            String systemUser = System.getProperty("user.name");
//            Path authenticationKeyFilePath = Path.of("/Users/" + systemUser + "/.emulator_console_auth_token");
//            String authenticationKey = Files.readString(authenticationKeyFilePath);
//            System.out.println("Authenticating emulator-" + emuPort);
//            String authCommand = "auth" + " " + authenticationKey;
//            // write authentication command with authentication key
//            BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(confConnection.getOutputStream()));
...
//            writer.flush();
            ...
            //System.out.println("[checkAuthentication -> contains is not working");
            System.out.println("Warning: Connection to the emulator-" + emuPort + " do not need authentication!");
        }
    }
    //
        ...
        try {
            while ((line = reader.readLine()) != null) {
                if (line.equals("OK") || line.contains("host port already active")) {
                    // LYLA: 01/16/2023
                    //System.out.println("[checkPortRedirection -> Port reader correct");
                    //

                    return true;
                } else if (line.contains("port probably used by another program on host")) {
                    // LYLA: 01/16/2023
                    //System.out.println("[checkPortRedirection -> Port reader correct, but port probably used by another program on host");
                    //
                    ...
                    // LYLA: 01/16/2023
                    //System.out.println("[checkPortRedirection -> Port reader correct, but line has KO");
                    //System.out.println("[checkPortRedirection -> Line is: " + line);
                    //
            ...
            // LYLA: 01/16/2023
            //System.out.println("[checkPortRedirection -> Port reader exception");
            //
        }
        // LYLA: 01/16/2023
        //System.out.println("[checkPortRedirection -> Port reader correct, but none of cases was correct");
        //
