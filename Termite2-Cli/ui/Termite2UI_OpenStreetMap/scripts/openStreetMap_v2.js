...
let directionsService = null; // LYLA: Chack if you do not use it you can delete it.
...
function confButtonClicked() {
    ...
    if (eventactive) {
        ...
        console.log("[$confButtonClicked> Event Active");
        ...
        console.log("[$confButtonClicked> Event Active finished");
    } else {
        console.log("[$confButtonClicked> Event Is Not Active");

        //https://github.com/bmcbride/bootleaf/tree/master/assets/leaflet-groupedlayercontrol

        // nodestable1 = document.getElementById('nodestable');
        // groupstable1 = document.getElementById('groupstable');
        //
        // nodestable1.style.display = 'none';
        // groupstable1.style.display = 'none';


        // confModal = document.getElementById('configModal');
        // confModal.style.display = 'block';

        //https://stackoverflow.com/questions/38202399/bring-modal-to-front
        document.getElementById('configModal').style.display = 'block';

        /*var win =  L.control.window(map,{title:'UI configurations'});
        win.options.modal = 'true';
        win.container(confModalUILYLA);
        //win.content(confModalUILYLA);
        win.show();*/
        ...
        //  document.getElementById('configModall').style.display = 'block';
        // document.getElementById('configModall').style.visibility= 'true';
        // if(!document.getElementById('configModall').style.visibility){
        //     console.log('hellloooooo');
        // }

        console.log("[$confButtonClicked> Finished")
    }
}
...
function myMap(){
    ...
    map.on("contextmenu", function (e){
        ...
        // const latLngToLayerPointTest = map.latLngToLayerPoint(myPoint);
        // console.log("Clicked Casted X: " + latLngToLayerPointTest.x);
        // console.log("Clicked Casted Y: " + latLngToLayerPointTest.y);
        ...
        // LYLA: I added. I changed this part to change the mouse context menu position by dragging the map
        // https://github.com/gnijuohz/echarts-leaflet/issues/17
        //
    ...
    // LYLA: I added this handler to hide all menus
    map.on('dragstart', function (){hideAllMenus();});

    // LYLA: I added this handler to change the map context menu position by changing the
    ...
    // LYLA: I added this handler to handle map resize
    ...
    //
    //     const myPoint = L.latLng(lat1,lng1);
    //     //const myPoint2 = [lat,lng];
    //     const latLngToLayerPointTest2 = map.latLngToLayerPoint(myPoint);
    //     leftDivergence = latLngToLayerPointTest2.x;
    //     topDivergence = latLngToLayerPointTest2.y;
    //     // map.on("contextmenu",function (e){
    //     //
    //     //     lat = e.latlng.lat;
    //     //     lng = e.latlng.lng;
...
function setUrlPath(){
    ...
    if (fullUrl.indexOf('indexOpenStreetMap.html') !== -1) {
        ...
        // LYLA: There is no folder with WebPage because of that I changed it.
        const midURL = fullUrl.split('Termite2UI_OpenStreetMap/WebPage/');
        //const midURL = fullUrl.split('Termite2UI_OpenStreetMap/WEB-INF/');
        pageUrl = midURL[0];
...
function interpolateFullPath(route) {
    ...
    for (numCoords; numCoords !== 1; numCoords--) {
        ...
        /*
        * LYLA: Since I read the json object returned from OpenRoutService server and fill the route array
        * in createRout function the first value is different and contains string "LatLng"
        * To handel this at first round of the for loop I will use the marker lat and lng instead of the first value.
        * And for others, we select the first 2 sequential coordinates on the route.
        * We select the first 2 sequential coordinates on the route.
        * */
        if (firstRoundFor){
        ...
        if (distance < selectedDistance && numCoords === 2) {
            ...
            // LYLA: We use string of Lat and Lng in our code to set markers' lat and lng.
...
function printDistances(path) {
    // LYLA: Distance of the whole path
    ...
    //console.log("printDistances > Path length in meters is: " + pathMeters);
...
function processNodeMovementEvent(id, marker, snappedlat, snappedlng, finalMove) {
    ...
    dragCircle(id,[snappedlat,snappedlng]); //LYLA: added to move the circle with the marker
...
/**
 * LYLA: This function creates the routes!
...
function createRoute(nodeId, destination) {
    ...
    request.onreadystatechange = function () {
        if (this.readyState === 4) {
            ...
            //console.log("createRoute > testin distanceTo --> pathSizeMeters: " + pathSizeMeters);
...
function continueMarkerCreation(id, snappedlat, snappedlng){
    ...
    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation: ghabl az addTableEntry");

    addTableEntry(id, snappedlat, snappedlng); // Adds the new marker info to the data table view (in right side of the UI)

    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation: baad az addTableEntry");

    const markerNeighbors = getNeighbors(id);

    // LYLA-IMP: check this, if it does not work change the "marker.getLatLng().lat" and "marker.getLatLng().lng" to the "snappedlat" and "snappedlng" respectively.
...
function createAndBindCircle(id, circleLat, circleLng) {
    ...
    // LYLA: bayad vaghti marker ro tekoon midi in circle ro ham tekoon bedi! -> DONE: dragCircle
}

// LYLA: Moves the circle when the associated marker is dragged (I added this function).
...
function deleteAllMarkers(){
    ...
    // LYLA: I added this one, to hide the map menu after deleting the markers.
    hideAllMenus();
...
function getNeighbors(nodemoving) {
    ...
    if (strlength !== 1) {
        nodesInRange = nodesInRange.substring(0, nodesInRange.length - 1); //substring() extracts a part of a string
    }
    nodesInRange += ')';
    return nodesInRange;
}
...
function addTableEntry(nodeId, latitude, longitude) {
    ...
    //refreshEmusBtn();
    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation -> addTableEntry");
...
<!--              &lt;!&ndash; LYLA: 27 Dec 2022: I added to test &ndash;&gt;-->
...
function setEmulatorOptionsString(nodeId) {
    ...
    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString: ghable -");

    let emuString = '<option id="-">-</option>'; // default


    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString: baade -");

    // LYLA: 10/04/2023
    if (flagEmulatorsView){
        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString: ghable refreshEmusBtn");
        refreshEmusBtn();
        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString: baade refreshEmusBtn");
    }
...
function redrawDropFormOptions(nodeId) {
    ...
    //LYLA 27 Dec 2022: added to check if dropform${nodeId} exists
    if (selectElement == null){
        alert('[redrawDropFormOptions -> $ dropdown form does not exist');
    }
    ...
    // //LYLA 27 Dec 2022: added to check if emulators is null
    // if (emulators.length == 0){
    //     alert('[redrawDropFormOptions -> $ emulators length is zero');
    // }else {
    //     if (emulators == null){
    //         alert('[redrawDropFormOptions -> $ emulators is null');
    //     }else {
    //         let lenghth = emulators.length;
    //         alert('[redrawDropFormOptions -> $ emulators length is: ');
    //         alert(`${lenghth}`);
    //     }
    // }
    ...
    emulators.forEach((emu) => {
        ...
        // //LYLA 27 Dec 2022: added to check if dropform${nodeId} exists
        // alert('[redrawDropFormOptions -> $ adding option' + `${emu}`);
...
function changeCursor(type) {
    //console.log("changeCursor > Changing cursor ...");
            ...
            switch (cmdArray[0]) {
                ...
                case 'EventMove': // cmd, node id, destination lat, destination lng
                    if (cmdArray.length !== 4) { // check for number of arguments
                        ...
                        // LYLA-IMP: Check if parseFloat is needed or not.
...
function buildSaveNetworkString() {
    ...
    movementPaths.forEach((value, key) => { // eventpaths
        // LYLA-IMP: Should be checked if it works correctly
...
function openEmulatorsView() {
    // LYLA: I added to show the "emulatorsDataModal"
    // LYLA: I commented 10/04/2023
...
function refreshEmusBtn() {
    ...
    // LYLA: 10/04/2023
    alert("alan to refreshEmusBtn()");
    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn()");


    const emulatorsDataArray = sendPostRequest('refresh'); // LYLA: emulatorsDataArray -> response text

    // LYLA: I added else part to check if the returned emulators list is empty or not (27 Dec 2022)
    if (emulatorsDataArray != null) {
        // LYLA: 10/04/2023
        alert("alan to refreshEmusBtn(), emulator hast");
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() : ghable refreshEmulators");
        ...
        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() : baade refreshEmulators");

        // // LYLA: I added make sure the returned value is not empty (27 Dec 2022)
        // let refreshEmulatorsSplitted = emulatorsDataArray.split(',');
        // alert('[refreshEmusBtn -> $ Returned emulator array for the refresh request is NOT empty');
        // alert(emulatorsDataArray);
        // alert(refreshEmulatorsSplitted[0]);
    }
    else{

        // LYLA: 10/04/2023
        alert("alan to refreshEmusBtn(), emulator i vojood nadare");
...
//LYLA: This function sends an HTTP post request to the Termite2 server.
//LYLA: Input = command, Output: response text
function sendPostRequest(msg) {
    ...
    // LYLA- IMP: vaghti server nabashe error 404 mide va to "xhttp.send(msg);" minde. Nemidooni CommunicationServlet kojast!
    ...
    // LYLA: 27 Dec 2022 I added to catch 404 error
    // LYLA: This one catches 404 error successfully- tested
...
// LYLA: Input -> arrays of string
function refreshEmulators(emulatorsDataArray) {

    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() -> refreshEmulators");
    ...
    //if (JSON.stringify(emulatorsDataArray) === previousEmusData) {
        ...
        // // LYLA: 27 Dec 2022 added to check if always ends here
        // alert('[refreshEmulators -> $ Emulators are the same');
        // //let emulatorsNamesString = JSON.stringify(emulatorsDataArray);
        // //alert(`${emulatorsNamesString}`);
        // alert(emulatorsDataArray);

        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() -> refreshEmulators: tooye if e bargasht");

        return;
    }

    // // LYLA: 27 Dec 2022 added to check if always ends here
    // alert('[refreshEmulators -> $ Emulators are different');
    // //let emulatorsNamesString = JSON.stringify(emulatorsDataArray);
    // alert(emulatorsDataArray);
    // //alert(`${emulatorsNamesString}`);
    ...
    // LYLA: 10/04/2023 Commented to change next
    ...
    if (flagEmulatorsView){
        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() -> refreshEmulators: flag true, update emulators");
        emulators = newEmulatorIds;
        flagEmulatorsView = false;
        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() -> refreshEmulators: flag true, emulators updated return");
        return;

    }else {

        // LYLA: 10/04/2023
        alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() -> refreshEmulators: ghable refreshBinds va flag false");
...
// LYLA: Input -> array of strings (emulators data), Output -> emulators ID? or pc IPs
function setEmulatorsDataObjects(dataArray) {
    ...
    let newEmulatorIds = []; //LYLA: holds PCs' IP
    let pcIp = '';
    for (i = 0; i < dataArray.length; i++) {
        ...
        if (splitted.length === 1) { // pc ip
            ...
            // // LYLA 27 Dec 2022: added to check if tempArray is undefined
            // if (tempArray === undefined){
            //     alert('[setEmulatorsDataObjects -> $ tempArray is undefined. Something is wrong');
            // }

            newEmulatorIds.push(splitted[0]);
            tempArray.push(splitted); // LYLA: 27 Dec 2022 here, pcIp is also added to tempArray.

            // // LYLA 27 Dec 2022: added to check the pcIp value, I think sth is wrong since we do not give value to it
            // alert('[setEmulatorsDataObjects -> $ PC IP is: ');
            // alert(`${pcIp}`);
...
// LYLA: Updates emulators and markers (nodes) binds after updating emulators list.
function refreshBinds(newEmulators) {

    // LYLA: 10/04/2023
    alert("createMarker -> continueMarkerCreation -> addTableEntry -> setEmulatorOptionsString -> refreshEmusBtn() -> refreshEmulators -> refreshBinds");

    const oldBinds = binds;
    binds = new Map(); // resets binds

    // LYLA: 10/04/2023 I commented to change in next
    // oldBinds.forEach((value, key) => {
    ...
    // // LYLA 27 Dec 2022: added to check if newEmulators is empty
    // if (newEmulators.length == 0){
    //     alert('[refreshBinds -> $ newEmulators length is zero so emulators will be null');
    // }else {
    //     alert('[refreshBinds -> $ emulators are');
    //     alert(newEmulators);
    // }

    emulators = newEmulators;

    // LYLA: 10/04/2023 I commented to add next
    ...
    // LYLA: 10/04/2023 I commented to add to the above code
    //rebindUnchanged(oldBinds);
}
...
// LYLA: Shows the help modal. I added.
function helpBtn(){
    document.getElementById('informationModal').style.display = 'block';
}

// LYLA: Closes the help modal. I added.
